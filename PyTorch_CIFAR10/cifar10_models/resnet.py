...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer2 = self._make_layer(
            block, out_channels[1], layers[1], stride=2, dilate=replace_stride_with_dilation[0]
        )
        self.layer3 = self._make_layer(
            block, out_channels[2], layers[2], stride=2, dilate=replace_stride_with_dilation[1]
        )
        self.layer4 = self._make_layer(
            block, out_channels[3], layers[3], stride=2, dilate=replace_stride_with_dilation[2]
        )
        ...
        if bottleneck_dim > 0:
            if split_layer in [0, 1, 2]:
                in_dim = first_out
            if split_layer in [3, 4, 5]:
                in_dim = out_channels[0]
            elif split_layer in [6, 7]:
                in_dim = out_channels[1]
            elif split_layer in [8, 9]:
                in_dim = out_channels[2]
            elif split_layer in [10, 11]:
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x)
            if i == self.split_layer and self.compress is not None:
                x = self.compress(x)
                x = self.decompress(x)
    ...
    def split_forward_shredder(self, x, noise):
        for i, layer in enumerate(self.layers):
            x = layer(x)
            if i == self.split_layer:
                if self.compress is not None:
